...
def list_customer_events(email: str, limit: int = 30) -> list[dict[str, Any]]:
    ...
    A view card is excluded for a sharper version of the same reason. `meta.view` rows say "Kevin
    Stucky opened the proposal" and exist for the ESTIMATOR (api_portal_viewed); the row is written
    into the shared thread because that is the only place staff read, and the notification bell is
    where the customer would otherwise be told about their own click — a notification generated BY
    the act of reading, arriving because they read. Each filter is one predicate and every one of
    them has to be here: the bell has no idea who a card is for.

    AND `meta.internal` — the strongest of the three, added when the CRM cards arrived. Those rows
    are staff-only: "reassigned from Kyle to RJ", "Closed–Lost. Reason: Selected another
    contractor", "Hanz logged a call". `list_messages` already keeps them out of the customer's
    copy of the shared thread, but THIS query is a second, independent customer surface, and it
    selects exactly the shape those cards have — author_kind 'staff', msg_type 'system'. Without
    the predicate, filing a job as lost rang the customer's bell to tell them so. The view card
    sets both keys and so is caught twice over; `view` stays because it is the narrower claim (a
    row that belongs in the customer's thread but not in their bell) and the two rules are not the
    same rule."""
    return qall(
        ...
        "  and coalesce((q.meta->>'view')::boolean, false) = false "
        "  and coalesce((q.meta->>'internal')::boolean, false) = false "
...
def mark_viewed(proposal_id: str) -> bool:
    ...
    Returns True on the sent -> viewed TRANSITION, i.e. the first view of this send.

    It has to, because `last_viewed_at = now()` runs on EVERY call and the status flip runs
    once, so from the outside a first view and a reload were indistinguishable. The caller now
    emails staff and writes a card, and a blind write meant a customer leaving the tab open
    would have alerted the estimator on every poll — the return value is the only thing that
    makes "they opened it" an event rather than a stream.

    The old status is captured in the SAME statement rather than by a select-then-update. That
    closes the window that matters: a separate read leaves an application-level gap of a whole
    round trip in which anything — the customer's other tab, a re-send, staff closing the job —
    can move the status, so the fact we notify about is not the fact we wrote.

    It is a narrowed race and not a serialization guarantee, which is worth stating because the
    SQL looks like one. Under READ COMMITTED, two genuinely simultaneous statements both take
    their snapshot before either commits; the loser blocks on the row lock and re-checks the
    UPDATE's own quals, but `prev` keeps the value from its original snapshot, so both can report
    'sent'. Making that impossible would mean gating the UPDATE on `proposal_status = 'sent'` —
    which cannot be done here, because `last_viewed_at` has to move on EVERY view, transition or
    not. The residual cost is one duplicate notification on a dead heat, which is the right thing
    to trade: the alternative reports a transition the row did not make.

    UNGUARDED, unlike its neighbours here, and deliberately: the route that calls it is the one
    place that must absorb a database failure (see api_portal_viewed), and swallowing it here
    would report "no transition" for a write that never happened — which reads as a reload and
    would silently lose the notification for a real first view."""
    row = q1(
        "with prev as ("
        "  select proposal_status as old_status"
        "    from public.portal_proposals where proposal_id = %s"
        ") "
        "update public.portal_proposals p "
        "   set viewed_at       = coalesce(p.viewed_at, now()), "
        "       last_viewed_at  = now(), "
        "       cycle_viewed_at = case when p.proposal_status = 'sent' "
        "                              then coalesce(p.cycle_viewed_at, now()) "
        "                              else p.cycle_viewed_at end, "
        "       proposal_status = case when p.proposal_status = 'sent' "
        "                              then 'viewed' else p.proposal_status end, "
        "       updated_at      = now() "
        "  from prev "
    ...
    return (row or {}).get("old_status") == "sent"
