...
def test_the_thread_hides_the_card_from_the_customer_by_default(monkeypatch):
    """THE THREAD IS SHARED, which an earlier version of this test got wrong. `list_customer_events`
    above is the customer's BELL; the thread itself comes from `list_messages`, and the same rows
    render in the customer's portal AND in the staff drawer. So excluding the card from the bell was
    never enough — unfiltered, the customer reads "Kevin Stucky opened the proposal" in their own
    conversation.

    The default therefore EXCLUDES `meta.internal`. A new caller is far likelier to be
    customer-facing than not, and the failure modes are not comparable: a staff reader that forgets
    the flag loses a card it can find elsewhere, while a customer reader that forgets it shows a
    customer an internal note, which cannot be taken back."""
    seen = {}
    monkeypatch.setattr(db, "qall", lambda sql, params=(): seen.update(sql=sql) or [])
    ...
    assert "coalesce((meta->>'internal')::boolean, false) = false" in sql, (
        "list_messages does not exclude internal cards by default — the customer sees a card "
        "saying they opened their own proposal")


def test_the_card_is_still_there_for_STAFF(monkeypatch):
    """The other half, or the exclusion above is indistinguishable from never writing the card. The
    thread is the whole point — it is where staff read a project's history."""
    seen = {}
    monkeypatch.setattr(db, "qall", lambda sql, params=(): seen.update(sql=sql) or [])
    db.list_messages("pid-1", include_internal=True)
    sql = " ".join(seen["sql"].split())
    assert "meta->>'internal'" not in sql, (
        "the staff drawer still filters internal cards, so the view notification has nowhere to "
        "land and neither will the CRM cards that follow it")


def test_only_the_staff_reader_opts_in(monkeypatch):
    """The filter is worthless if the wiring is wrong, and the wiring is what a person edits. The
    customer's page and its poll must NOT pass include_internal; the staff drawer must."""
    import inspect
    import re
    ...
    for m in re.finditer(r"_customer_msg\(m[^)]*\)\s*\n?\s*for m in db\.list_messages\(([^)]*)\)", src):
        assert "include_internal" not in m.group(1), (
            "a customer-facing reader opted into internal cards: %r" % m.group(1))
...
def test_the_card_carries_both_meta_flags(portal):
    """TWO keys, doing two different jobs, and the card needs both.

    `internal` keeps the row out of the customer's copy of the shared THREAD (db.list_messages
    excludes it by default). `view` keeps it out of the customer's notification BELL
    (db.list_customer_events), and also leaves the row identifiable as a view rather than as any
    internal card, which the CRM cards to come will also be.

    Drop `internal` and the customer reads "Kevin opened the proposal" in their own conversation.
    Drop `view` and they get a bell notification generated by their own click."""
    _view(portal)
    assert portal.calls["cards"][0]["meta"] == {"view": True, "internal": True}
